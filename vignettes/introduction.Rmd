...
can be placed in any order in the piped command chain and each one relies (for the most part) on [non-standard evaluation](https://CRAN.R-project.org/package=dplyr/vignettes/nse.html) for its arguments. This means that you don't have to quote variable names.
